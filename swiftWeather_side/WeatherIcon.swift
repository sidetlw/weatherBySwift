...
struct WeatherIcon {
    enum IconType:String ,CustomStringConvertible{
        ...
        var description:String {
            ...
            case IconType.day201: return "\u{f010}"
    ...
    init(code:Int,dayORnight:String) {
        var rawValue:String?
        if dayORnight.rangeOfString("n") != nil {
            rawValue = String("night") + String(code)
        }
        else  {
            rawValue = String("day") + String(code)
        }
        guard let iconType = IconType(rawValue: rawValue!) else {
            iconString = ""
            return
        }
        iconString = iconType.description
    }//init
}